...
class OrdersPage
{
    ...
    {
        return $this->orderManager->selectOrders();
    }

    public function getOrderItems($orderId): array
    {
        return $this->orderManager->selectOrderProducts($orderId);
    }
}
            ...
            <div class="side-navbar d-flex justify-content-between flex-wrap flex-column" id="sidebar">
                <ul class="nav flex-column w-100" id="menu-navigation-list">
                    ...
                    <li onclick="" class="nav-link text-center mb-4">
                        <span class="nav-item-text active mx-2">Narudžba</span>
                    </li>
        ...
        <section class="container products-containter" role="main">
            <table class="table table-bordered table-hover text-center align-middle mt-5" id="table-products">
                ...
                <tr>
                    ...
                    <th scope="col">Naručeni proizvodi</th>
                </tr>
                </thead>
                <tbody>
                <?php foreach ($orders as $row) : ?>
                    <tr class="row-product">
                        ...
                        <td><?= htmlspecialchars($row['cityPostalCode']) ?></td>
                        <td><?= htmlspecialchars($row['phoneNumber']); ?></td>
                        <td>
                            <?php $orderItems = $ordersPage->getOrderItems($row['id']);
                            foreach ($orderItems as $item) : ?>
                                <p><?= htmlspecialchars($item['quantity']) ?>x
                                    <?= htmlspecialchars($item['name']) ?></p>
                            <?php endforeach; ?>
                            <p>
                                Ukupno: <?= htmlspecialchars(number_format(array_reduce(array_map('getProductTotal', $orderItems), 'sum'), 2, ',')) ?>
                                kn
                            </p>
                        </td>
                    </tr>
