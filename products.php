...
$products = $productsPage->getUserProducts();
?>
    ...
    <body>
        ...
        <section class="container products-containter" role="main">
            <div class="row justify-content-end">
                <form class="d-flex flex-row-reverse" action="product.php" method="GET">
                    <button class="col-auto btn-primary btn btn-primary-thin" type="submit">Dodaj novi proizvod
                    </button>
                </form>
            </div>
            ...
            <table class="table table-bordered table-hover text-center align-middle mt-5" id="table-products">
                ...
                <?php foreach ($products as $row) : ?>
                    <tr class="row-product">
                        <td class="product-id" style="display:none;"><?= htmlspecialchars($row['id']) ?></td>
                        <td>
                            <img src="<?= htmlspecialchars($row['image_url']); ?>"/>
                        </td>

                        <td><?= htmlspecialchars($row['name']) ?></td>
                        <td><?= htmlspecialchars(number_format($row['price'], 2, ',')) ?> kn</td>
                        <td><?= htmlspecialchars($row['quantity']); ?></td>

                        <td>
                            <form action="product.php" method="POST" name="updateForm">
                                <input type="hidden" name="productId" value=<?= htmlspecialchars($row['id']) ?>/>
                                <button class="btn-primary btn btn-primary-thin btn-update" type="submit">Ažuriraj
                                    proizvod
                                </button>
                            </form>
                        </td>
